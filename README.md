# CPE 513 — Problem P9: SMS spam filtering

An empirical comparison of five learning algorithms on the UCI SMS Spam Collection,
under one feature pipeline, one fold structure and one tuning budget.

---

## READ THIS FIRST

Two things you must do before you submit.

**1. Fill in the placeholders.** The report has five, all in square brackets:
`[YOUR NAME]`, `[YOUR ID]`, `[DATE]`, `[DOWNLOAD DATE]`, `[YOUR REPOSITORY LINK]`.
They are in `src/build_report.py` (search for `[YOUR`). Edit them there, then run
`python src/build_report.py` and re-render the PDF — do not edit the PDF directly,
because the whole point of the build script is that the report is generated, not typed.

**2. Re-run it yourself and read the code.** Your brief says you may use AI assistants
but *"may not submit work you cannot explain or reproduce"*, and there is a 15-minute
oral check where you may be asked to explain the fold boundary in your pipeline,
justify one hyperparameter choice, and regenerate a result live. The
"Oral check preparation" section at the bottom of this file is written for exactly
that. Budget an hour or two. The AI-use statement in Appendix B of the report
declares that an assistant drafted the scripts and prose and that you directed the
design and verified the output — running the pipeline end to end yourself is what
...
data/SMSSpamCollection     the raw dataset, unmodified (MD5 1949b64a224790d01335c2bf8a0e48b2)
src/run_experiments.py     runs the nested cross-validation (the experiment)
src/analyse_results.py     aggregation, statistical tests, figures
src/build_report.py        builds the report from the archived CSVs
report/report.pdf          THE REPORT — 8 pages (main body 6.5, under the 10-page limit)
report/report.html         its source
results/*.csv              raw per-fold results and every derived table
figures/*.png              Figures 1-3
```
